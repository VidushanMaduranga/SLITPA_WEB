...
if (isset($_POST['login'])) {
    ...
    $username = filter_input(INPUT_POST, 'username', FILTER_SANITIZE_FULL_SPECIAL_CHARS); // Sanitize input
    $password = $_POST['password']; //  DO NOT store plain passwords

    if (empty($username) || empty($password)) {
        //  Check if both fields are filled
        $login_error = "Username and password are required.";
    } else {
        //  Both fields are filled, so let's check the database.
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            exit(); //  Stop execution here after the redirect.
        } else {
            //  Invalid login
...
if (isset($_POST['add_event'])) {
    ...
    if (empty($title) || empty($description) || empty($location) || empty($event_date)) {
        ...
        $stmt = $conn->prepare("INSERT INTO events (title, description, location, event_date, end_date) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $title, $description, $location, $event_date, $end_date);
        if ($stmt->execute()) {
...
<body>
    ...
    <div class="container py-5">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <button type="submit" name="<?= $edit_event ? 'update_event' : 'add_event' ?>" class="btn btn-primary"><?= $edit_event ? 'Update Event' : 'Add Event' ?></button>
